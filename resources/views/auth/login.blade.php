...
<body class="bg-zinc-950 text-white min-h-screen flex items-center justify-center px-4">
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        <div class="fade-up mb-10 text-center" style="animation-delay: 0s">
            ...
            <p class="text-zinc-500 text-sm mt-2">Welcome back. Say something.</p>
        </div>
        ...
        <div class="fade-up glow bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-5" style="animation-delay: 0.1s">
            ...
            {{-- Session Status --}}
            @if (session('status'))
                <div class="text-green-400 text-sm text-center">{{ session('status') }}</div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div class="space-y-1">
                    <div class="flex justify-between items-center">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-xs text-zinc-500 hover:text-white transition">Forgot?</a>
                        @endif
                    </div>
                    <input
                        type="password"
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="remember" id="remember" class="accent-white w-4 h-4">
        ...
        <div class="fade-up text-center mt-6" style="animation-delay: 0.2s">
            <p class="text-zinc-500 text-sm">
                ...
                <a href="{{ route('register') }}" class="text-white hover:underline ml-1">Sign up</a>
            </p>
        </div>
